...
dependence of input parameters to problem solving. Also reasons of effect will
be explained if it's be possible.

## Formulation of the Navigation Problem of Speed

It is boat on water surface which has to catch the immovable target placed on
...
$x^* = l \times cos(\phi), y^* = l \times sin(\phi)$
...
and $v>s(y^*)$ are must to boat can reach such radius of $y^*$ where boat boat
speed will exceed stream speed in each point on way to target, so boat will be
approaching to target on it's way. 
...
$y=\int_0^\infty u(t)dt < y^*$, where $y$ is boat coordinate. That's why named
...
which could be noticed earlier. But first about dependence. As we can see time
of boat reaching target depends nonlinear on stream speed. When initial stream
speed approaches to such that whole stream spead near to speed of boat the time
...
integral properties 
$$\exists I_1 \equiv \int_{0}^{\hat{y}}s(y)dy$$
$$\exists I_2 \equiv \int_{\hat{y}}^{y^*}s(y)dy$$
and hence $$I = I_1 + I_2 = \int_{0}^{y^*}s(y)dy$$
